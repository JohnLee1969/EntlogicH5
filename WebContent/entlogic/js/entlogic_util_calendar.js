 ...
 * @returns
 */
function entlogic_util_calendar() {
	 ...
	 */
	this.getNL = function(str) {
		var thisDate= Lunar(str);
		return cyclical(thisDate.year-1900+36);
	};
	
	/**
	 * 根据公历日期字符串计算农历属性
	 */
	this.getAnimal = function(str) {
		var thisDate= Lunar(str);
		return Animals[(thisDate.year-4)%12];
	};

	/**
	 * 根据生日的月份和日期，计算星座。
	 */
	this.getAstro = function(str) {
		var thisDate= NewDate(str);
		var month = thisDate.month;
		var day =  thisDate.day;
 ...
 * 实例化日期时间函数包
 */
EntlogicUtilCalendar = new entlogic_util_calendar();