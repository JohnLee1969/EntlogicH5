...
function btnLogin_click() {
    ...
    if (dboHS_ENTERPRISE.dataTable.getSize() > 0) {
        ...
        popUpMobError("错误提示", "您还没有完成新企业账户注册，请继续设置密码！！");
        jumpTo("RegisterEnterpriseForm1.html?enterpriseOid=" + enterpriseOid);
        return;
    }
    if (loginType == "E") {
        ...
        if (dboHS_ENTERPRISE.dataTable.getSize() > 0) {
           	...
           	localStorage.setItem("userPassword", txtPassword.getValue());
            var enterpriseOid = dboHS_ENTERPRISE.dataTable.getRecord(0).getValue("OID");           
           	setUserSessionItem("enterpriseOid", enterpriseOid);         
            removeUserSessionItem("directorOid");
            jumpTo("MainForm.html");
            return;
        } else {
            popUpMobError("错误提示", "登录账号或密码错误！！");
...
function btnGotoRegisterEnterprise_click() {
    localStorage.removeItem("enterpriseOid");
...
function btnFindBackPassword_click() {
    jumpTo("FindBackPasswordForm0.html");
};
